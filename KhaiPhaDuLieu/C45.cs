...
namespace KhaiPhaDuLieu
{
    public class C45
    {
        ...
        public C45(List<List<string>> Examples, List<Attribute> Attributes)
        {
        ...
        private double Gain(List<List<string>> Examples, Attribute A, string bestat)
        {
            ...
            for (int i = 0; i < Examples.Count; i++)
            {
                ...
                if (Examples[i][Examples[0].Count - 1] == "dau")
                {
                    CountPositives++;
                    CountPositivesA[j]++;
                }
        ...
        private TreeNode GiaiThuatC45(List<List<string>> Examples, List<Attribute> Attribute, string bestat)
        {
            Solution = Solution + "************* Xét " + "{ " + bestat + " }" + " *************";
            /*if (CheckAllPositive(Examples))
            {
                Solution += "\n Tất cả các mẫu đều khẳng định => Trả về nút gốc với nhãn < Dau >";
                return new TreeNode(new Attribute("Dau"));
            }
            if (CheckAllNegative(Examples))
            {
                Solution += "\n Tất cả các mẫu đều phủ định => Trả về nút gốc với nhãn < Rot >";
                return new TreeNode(new Attribute("Rot"));
            }
            ...
            for (int i = 0; i < BestAttribute.Value.Count; i++)
            {
                ...
                else
                {
                    ...
                    Root.AddNode(GiaiThuatC45(Examplesvi, Attribute, BestAttribute.Value[i]));
                }
            }
            return Root;
        }
        ...
        private bool CheckAllPositive(List<List<string>> Examples)
        {
            for (int i = 0; i < Examples.Count; i++)
            {
                if (Examples[i][Examples[0].Count - 1].ToString() == "rot")
                    return false;
            }
            return true;
        }
        ...
        public void GetTree()
        {
            ...
            Tree = GiaiThuatC45(Examples, at, "Bộ DLHL");
            Depth = GetDepth(Tree);
        }
